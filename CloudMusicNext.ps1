...
public static class CloudMusicNextNative
{
...
foreach ($process in @(Get-Process -Name cloudmusic -ErrorAction SilentlyContinue)) {
    ...
    [CloudMusicNextNative]::EnumWindows({
        param($hWnd, $lParam)
        [uint32]$windowProcessId = 0
        [void][CloudMusicNextNative]::GetWindowThreadProcessId($hWnd, [ref]$windowProcessId)
        if ($windowProcessId -eq $candidateProcessId -and [CloudMusicNextNative]::ReadClassName($hWnd) -eq "icon") {
...
$before = [CloudMusicNextNative]::ReadWindowText($foundWindow)
$sent = [CloudMusicNextNative]::SendCommand($foundWindow, $commandSlot)
Start-Sleep -Milliseconds 750
$after = [CloudMusicNextNative]::ReadWindowText($foundWindow)
