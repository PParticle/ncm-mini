...
public static class CloudMusicPreviousNative
{
...
foreach ($process in @(Get-Process -Name cloudmusic -ErrorAction SilentlyContinue)) {
    ...
    [CloudMusicPreviousNative]::EnumWindows({
        param($hWnd, $lParam)
        [uint32]$windowProcessId = 0
        [void][CloudMusicPreviousNative]::GetWindowThreadProcessId($hWnd, [ref]$windowProcessId)
        if ($windowProcessId -eq $candidateProcessId -and [CloudMusicPreviousNative]::ReadClassName($hWnd) -eq "icon") {
...
$before = [CloudMusicPreviousNative]::ReadWindowText($foundWindow)
$sent = [CloudMusicPreviousNative]::SendCommand($foundWindow, $commandSlot)
Start-Sleep -Milliseconds 750
$after = [CloudMusicPreviousNative]::ReadWindowText($foundWindow)
